...
router.post(
  ...
  async (req, res) => {
    try {
      const { firstname,middlename,lastname,emailid,businessname,businessaddress,number,businessDescription,pincode } = req.body;
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
      }
      const note = new Notes({ firstname,middlename,lastname,emailid,businessname,businessaddress,number,businessDescription,pincode, user: req.user.id });
